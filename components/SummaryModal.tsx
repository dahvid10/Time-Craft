...
export const SummaryModal: React.FC<SummaryModalProps> = ({ summary, isLoading, onClose }) => {
  const [htmlContent, setHtmlContent] = useState('');
  const [isCopied, setIsCopied] = useState(false);

  useEffect(() => {
    if (summary) {
      const parsedHtml = marked.parse(summary, { gfm: true, breaks: true });
        ...
        <div className="p-6 border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
            <div className="flex items-center justify-between gap-3">
                <div className='flex items-center gap-3'>
                    ...
                    <h2 className="text-xl font-bold text-teal-600">Plan Summary</h2>
                </div>
                {summary && !isLoading && (
                    <button
                        onClick={handleCopy}
        ...
        <div className="p-6 overflow-y-auto">
          {isLoading && (
            <div className="min-h-[150px] flex flex-col items-center justify-center text-center animate-pulse">
                <div className="text-sky-600 dark:text-sky-400 font-semibold">Generating your plan summary...</div>
                <div className="text-gray-500 dark:text-slate-400 mt-2">The AI is analyzing your schedule's scope and efficacy.</div>
            </div>
          )}

          {summary && (
             <div 
                className="prose prose-sm max-w-none text-gray-700 dark:text-slate-300 prose-headings:text-teal-500 prose-ul:list-disc prose-ul:ml-4 prose-p:my-2 prose-li:my-2 prose-strong:text-gray-800 dark:prose-strong:text-slate-100"
